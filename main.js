...
function storeTheme(themeName) {
    localStorage.setItem("setTheme", themeName);
}

// For restoring theme, if selected by the user in the past
function restoreTheme() {
    let restoredTheme = localStorage.getItem("setTheme");

    if (restoredTheme) {

    document.documentElement.className = `theme-${restoredTheme}`;

    toggleButtonSelection(restoredTheme, true);
...
function restoreName() {
    let restoredName = sessionStorage.getItem("displayName");

    if (restoredName) {
        setInputValue("display-name", restoredName);
    }
